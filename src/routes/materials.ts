...
app.get("/", async (c) => {
  const includeContent =
    c.req.query("includeContent") === "true" || c.req.query("full") === "1";
  ...
  const result = await db
    .select({
      ...
      tool_type: materials.tool_type,
      discover_tool_type: materials.discover_tool_type,
      discover_tool_url: materials.discover_tool_url,
      explore_tool_type: materials.explore_tool_type,
      explore_tool_url: materials.explore_tool_url,
      launch_tool_type: materials.launch_tool_type,
      launch_tool_url: materials.launch_tool_url,
      transform_tool_type: materials.transform_tool_type,
      transform_tool_url: materials.transform_tool_url,
      assess_tool_type: materials.assess_tool_type,
      assess_tool_url: materials.assess_tool_url,
    })
    .from(materials);
  return c.json(result);
});
...
app.get("/:id/quiz", async (c) => {
  ...
  const quiz = await db
    .select()
    .from(quizzes)
    .where(eq(quizzes.material_id, id))
    .limit(1);
  ...
  const qs = await db
    .select()
    .from(questions)
    .where(eq(questions.quiz_id, quizData.id));
